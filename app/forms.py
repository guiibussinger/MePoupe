...
class ReceitaForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if dataValidacao:
            if validada:
                raise forms.ValidationError("Nao pode auto-validar se quiser validar numa data futura!")
            if dataValidacao < datetime.date.today():
                raise forms.ValidationError("A data de validacao nao pode ser anterior a data atual!")
...
class DespesaForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        if dataValidacao:
            if validada and dataValidacao:
                raise forms.ValidationError("Nao pode auto-validar se quiser validar numa data futura!")
            elif dataValidacao < datetime.date.today():
                raise forms.ValidationError("A data de validacao nao pode ser anterior a data atual!")
